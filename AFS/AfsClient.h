...
class AfsClient {
    ...
    int rpc_rename(const char * from,
    ...
    int rpc_utimens(const char * path,
        const struct timespec * ts, struct fuse_file_info * fi) {
    ...
    int rpc_putFile(const char * path) {
        ...
        std::unique_ptr < ClientWriter < FileContent >> writer(stub_ -> afsfuse_putFile( & context, & returnedFile));
        try {
            FileReaderIntoStream < ClientWriter < FileContent > > reader(string(path), * writer);
            ...
            std::cerr << "Failed to send the file " << path << ": " << ex.what() << std::endl;
        ...
        if (!status.ok()) {
            ...
            std::cout << "Finished sending file : " << path << std::endl;
        }
	...
	File requestedFile;
        ...
        try {
            ...
            if (!status.ok()) {
                ...
                std::cerr << "with filename " << path << ": " << status.error_message() << std::endl;
                return false;
            }
